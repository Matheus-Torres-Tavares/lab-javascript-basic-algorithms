...
let etfind = latinp.match(/et/g).length;
...
function palindrome(str) {
  var cleanup = /[^A-Za-z0-9]/g;
  var lowerCaseClean = str.toLowerCase().replace(cleanup,'');
  var palLength = str.length;
  for (i=0; i <palLength/2; i++) {
    if (str[i] !== str[palLength-1-i]){
      return false
    }
  }return true;
}
palindrome("reinier reinier reiniert")